...
<?php 
    ...
    if (ICL_LANGUAGE_CODE == 'es') {
        $objCategory = get_category_by_slug('noticias');        
    } elseif(ICL_LANGUAGE_CODE == 'en') {
        $objCategory = get_category_by_slug('news');
    } elseif(ICL_LANGUAGE_CODE == 'pt-br') {
        $objCategory = get_category_by_slug('noticias-pt');
    }
?>
    <div class="wrapper-container">
        <div class="page container container-me-pagexx color-bg-white ">
            <div class="subtitle subtitle-bg-2">
                <h2><?php _e($objCategory->name, 'andexone') ?></h2>
            </div>
...
<?php
    ...
    if (is_object($objCategory)) {
        ...
        $category_id = $objCategory->cat_ID;
...
<?php if (is_array($dataPost) && count($dataPost) > 0): ?>
    ...
    <?php endforeach;?>
<?php else: ?>
    <h3><?php _e('No found result in this moment', 'andexone') ?>.</h3>
<?php endif; ?>
